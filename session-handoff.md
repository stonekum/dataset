...
> 上一轮：Session 001（2026-05-22）。一口气从空仓库推进到所有 8 个功能交付（F07 按用户决策降级为 CSV/Markdown 下载并标记 deferred，其余 7 个 status=done）。
...
1. **F07 真 PDF 实现**：reportlab + plotly+kaleido，把现在 Markdown 摘要转成正式排版的 PDF
2. **真实数据接入**：人工导出一次 Metricool CSV，确认指纹匹配；若有偏差按真实表头更新 `_METRICOOL_COLUMN_MAP`
3. **示例数据补 posts_count**：让 KPI "时段发帖数" 有实际值
4. **写 pytest**：把本轮散在 bash 里的 AppTest/边界用例固化下来
